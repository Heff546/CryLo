# The Current/Future Status of ZMQ in CryLo

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. CryLo
...
matching supported by both CryLo and ZMQ. The `format`, `context` and `event`
...
The CryLo daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The CryLo daemon also ensures that
...
The CryLo daemon will send a `txpool_add` pub exactly once for each
